...
mapping = {'none':0, 'a few':1, 'some':2, 'a lot':3}
average_smoking = np.mean([mapping[item] for item in survey_responses])
...
male_savings = [np.mean([savings[i] for i in [0, 3, 4]])]
female_savings = [np.mean([savings[i] for i in [1, 2, 5]])]
