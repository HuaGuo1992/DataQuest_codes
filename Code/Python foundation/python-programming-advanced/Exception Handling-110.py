...
class Trial():
    def __init__(self, trial):
        self.efficiency = float(trial[0])
        self.individual = int(trial[1])
        self.chopstick_length = int(trial[2])
...
class Chopstick(object):
    def __init__(self, length):
        ...
        for datarow in chopsticks:
            if int(datarow[2]) == self.length:
                self.trials.append(datarow)
...
chopstick_list = [Chopstick(num) for num in chopstick_lengths]
